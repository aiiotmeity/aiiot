...
def health_assessment_api(request):
    """API endpoint for health assessment submission"""
    try:
        data = json.loads(request.body)
        ...
        # FIX: Handle multiple users with same name
        try:
            users = User.objects.filter(name=username)
            user_count = users.count()
            
            print(f"DEBUG: Found {user_count} users with name: {username}")
            ...
            if user_count == 0:
                ...
                # Multiple users found - use the most recent one
                user = users.order_by('-id').first()
                print(f"DEBUG: Multiple users found, using most recent: {user.id}")
        ...
        # Calculate health score using our function
        health_score = calculate_health_score_from_data(data)

        print(f"DEBUG: Processing data for user: {user.name} (ID: {user.id})")
        print(f"DEBUG: Calculated health score: {health_score}")

        # FIX: Better handling of unique constraint
        try:
            # Try to get existing assessment for this specific user
            assessment = HealthAssessment.objects.filter(user=user).first()
            
            if assessment:
                # Update existing assessment
                assessment.age_group = age_group
                assessment.gender = gender
                assessment.respiratory_conditions = respiratory_conditions
                assessment.smoking_history = smoking_history
                assessment.living_environment = living_environment
                assessment.common_symptoms = common_symptoms
                assessment.occupational_exposure = occupational_exposure
                assessment.medical_history = medical_history
                assessment.health_score = health_score
                assessment.save()
                created = False
                print(f"DEBUG: Updated existing assessment: {assessment.id}")
            else:
                ...
                created = True
                print(f"DEBUG: Created new assessment: {assessment.id}")

        except Exception as e:
            print(f"DEBUG: Error saving assessment: {e}")
            return Response({'error': f'Failed to save assessment: {str(e)}'}, 
                           status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        return Response({
            ...
            'created': created,
            'username': user.name,  # Include username instead of user_id
        ...
        import traceback
        traceback.print_exc()
        return Response({'error': f'Failed to save health assessment: {str(e)}'}, 
                       status=status.HTTP_500_INTERNAL_SERVER_ERROR)
